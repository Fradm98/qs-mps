        ...
        if sweep == "right":
            ...
            if trunc_tol:
                ...
                if site == self.L // 2:
                    ...
                    np.savetxt(
                        f"C:/Users/HP/Desktop/mps/results/times_data/schmidt_values_middle_chain_{self.model}_L_{self.L}_chi_{self.chi}_h_{self.h:.{precision}f}",
                        s,
                    )
            elif trunc_chi:
                s_trunc = s[: self.chi]
            ...
            u, s, v = np.linalg.svd(m, full_matrices=False)
            np.savetxt(
                f"C:/Users/HP/Desktop/mps/results/times_data/update_site_{site}_h_{self.h:.2f}",
                [time.perf_counter() - time_svd],
            )
            # print(f"Time of svd during update state during sweeping {sweep} for site {site}: {time.perf_counter()-time_svd}")
            if trunc_tol:
    ...
    def sweeping(
        ...
        for n in range(n_sweeps):
            ...
            middle_chain = np.loadtxt(
                f"C:/Users/HP/Desktop/mps/results/bonds_data/schmidt_values_middle_chain_{self.model}_L_{self.L}_chi_{self.chi}_h_{self.h:.{precision}f}"
            )
            ...
            if s_min < e_tol:
                ...
                np.savetxt(
                    f"C:/Users/HP/Desktop/mps/results/energy_data/energies_sweeping_{self.model}_two_charges_L_{self.L}_chi_{self.chi}_h_{self.h:.{precision}f}",
                    energies,
                )
                if var:
                    np.savetxt(
                        f"C:/Users/HP/Desktop/mps/results/energy_data/variances_sweeping_{self.model}_L_{self.L}_chi_{self.chi}_h_{self.h:.{precision}f}",
                        variances,
                    )
        ...
        if sweep == "right":
            ...
            if trunc_tol:
                ...
                if site == self.L // 2:
                    # print(f'Schmidt values:\n{s}')
                    np.savetxt(
                        f"C:/Users/HP/Desktop/mps/results/bonds_data/schmidt_values_middle_chain_{self.model}_L_{self.L}_chi_{self.chi}_trotter_step_{trotter_step}_delta_{delta}",
                        s,
                    )
            elif trunc_chi:
                s_trunc = s[: self.chi]
