...
parser.add_argument(
    "-m",
    "--model",
    help="Model to simulate, available are 'Ising', 'Z2', 'ANNNI', 'Cluster', 'XXZ'. By default Cluster",
    default="Cluster",
    type=str,
)
parser.add_argument(
    "-e",
    "--eps",
    help="Weight to the degeneracy breaking term",
    default=1e-5,
    type=float,
)
parser.add_argument(
    "-w",
    "--which",
    help="Number of sites for the reduced density matrices. It will be centered in the middle of the chain",
    default=1,
    type=int,
)
parser.add_argument(
    "-d",
    "--deg",
    help="Degeneracy method used during the DMRG. Depends on the model used",
    default=1,
    type=int,
)
parser.add_argument(
...
if args.model == "Ising":
    ...
    raise SyntaxError(
        "Model not valid. Choose among 'Ising', 'Z2', 'ANNNI', 'Cluster', 'Cluster-XY', 'XXZ'"
    )
...
for L in args.Ls:
    ...
    for chi in args.chis:  # L // 2 + 1
        ...
        for hy in interval_y:
            ...
            for hx in interval_x:
                ...
                if args.model == "Ising" or args.model == "Cluster":
                    chain = MPS(
                        L=L, d=d, chi=chi, model=args.model, eps=args.eps, h=hx, J=hy
                    )
        ...
        rdms_chi = np.asarray(rdms_chi).reshape(
            (len(interval_x), len(interval_y), d**args.which, d**args.which)
        )
        rdms_chi = rdms_chi[::-1]
        np.save(
            f"{parent_path}/results/rdms_data/{args.which}_sites-rdms_dmrg_{args.model}_L_{L}_hx_{args.hx_i}-{args.hx_f}_hy_{args.hy_i}-{args.hy_f}_delta_{args.npoints}_degeneracy_method_{args.deg}_eps_{args.eps}_guess_path_chi_{chi}.npy",
            rdms_chi,
        )
