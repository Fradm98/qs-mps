...
parser.add_argument(
    "-op", "--operator", help="operator for the magnetization", default="X", type=str
)
...
for L in args.Ls:
    for chi in args.chis:
        ...
        for h in interval:
            for j in interval:
                chain_mps = MPS(L=L, d=d, model=args.model, chi=chi, h=h, J=j)

                chain_mps.load_sites(path=path_tensor, precision=precision)

                if args.o == "mag":
                    print(f"Magnetization for h:{h:.{precision}f}, j:{j:.{precision}f}, L:{L}")
                    op = args.operator
    ...
    if args.o == "mag":
        ...
        np.save(
                    f"{parent_path}/results/mag_data/mag_{op}_{moment}_moment_{args.model}_L_{L}_h_{args.h_i}-{args.h_f}_delta_{args.npoints}_chi_{chi}.npy",
                    M,
                )
